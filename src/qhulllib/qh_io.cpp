...
#include "qinterface.h" // the convex hull virtual interface for reporting results.
...
void qh_produce_output(void)
{
  ...
  {
    gInterface->HullPrintf (qh ferr, "qhull internal error (qh_produce_output): temporary sets not empty (%d)\n", qh_setsize ((setT*)qhmem.tempstack));
...
pointT *qh_detvnorm (vertexT *vertex, vertexT *vertexA, setT *centers, realT *offsetp) {
  ...
  if (numcenters > dim) {
    ...
    gInterface->HullPrintf(qh ferr, "qh_detvnorm: too few points (%d) to compute separating plane\n", numcenters);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
  i= 0;
...
setT *qh_detvridge3 (vertexT *atvertex, vertexT *vertex) {
  ...
  if (qh CHECKfrequently) {
    FOREACHneighbor_(vertex) {
      if (!neighbor->seen2) {
	gInterface->HullPrintf (stderr, "qh_detvridge3: neigbors of vertex p%d are not connected at facet %d\n",
...
void qh_geomplanes (facetT *facet, realT *outerplane, realT *innerplane) {
  ...
  if (qh MERGING || qh JOGGLEmax < REALmax/2) {
    ...
    if (qh JOGGLEmax < REALmax/2)
      radius -= qh JOGGLEmax * (double)sqrt ((double)qh hull_dim);  /* already accounted for in qh_outerinner() */
    *outerplane += radius;
    *innerplane -= radius;
...
void qh_order_vertexneighbors(vertexT *vertex) {
  ...
  while (qh_setsize (vertex->neighbors)) {
    ...
    if (!neighbor) {
      gInterface->HullPrintf (qh ferr, "qhull internal error (qh_order_vertexneighbors): no neighbor of v%d for f%d\n",
        vertex->id, facet->id);
...
void qh_printafacet(FILE *fp, int format, facetT *facet, boolT printall) {
  ...
  case qh_PRINTarea:
    if (facet->isarea) {
      gInterface->HullPrintf (fp, qh_REAL_1, facet->f.area);
      gInterface->HullPrintf (fp, "\n");
    }else
      gInterface->HullPrintf (fp, "0\n");
    break;
  case qh_PRINTcoplanars:
    gInterface->HullPrintf (fp, "%d", qh_setsize (facet->coplanarset));
    FOREACHpoint_(facet->coplanarset)
      gInterface->HullPrintf (fp, " %d", qh_pointid (point));
    gInterface->HullPrintf (fp, "\n");
    break;
  ...
  LABELprintnorm:
    if (!facet->normal) {
      gInterface->HullPrintf (fp, "no normal for facet f%d\n", facet->id);
      break;
    }
    if (qh CDDoutput) {
      gInterface->HullPrintf (fp, qh_REAL_1, -offset);
      for (k=0; k < qh hull_dim; k++) 
	gInterface->HullPrintf (fp, qh_REAL_1, -facet->normal[k]);
    }else {
      for (k=0; k < qh hull_dim; k++) 
	gInterface->HullPrintf (fp, qh_REAL_1, facet->normal[k]);
      gInterface->HullPrintf (fp, qh_REAL_1, offset);
  ...
  case qh_PRINTneighbors:
    gInterface->HullPrintf (fp, "%d", qh_setsize (facet->neighbors));
    FOREACHneighbor_(facet)
      gInterface->HullPrintf (fp, " %d",
  ...
  case qh_PRINTpointintersect:
    if (!qh feasible_point) {
      gInterface->HullPrintf (fp, "qhull input error (qh_printafacet): option 'Fp' needs qh feasible_point\n");
  ...
  LABELprintinfinite:
    for (k= qh hull_dim; k--; )
      gInterface->HullPrintf (fp, qh_REAL_1, qh_INFINITE);
    gInterface->HullPrintf (fp, "\n");   
    break;
  case qh_PRINTpointnearest:
    FOREACHpoint_(facet->coplanarset) {
      ...
      gInterface->HullPrintf (fp, "%d %d %d " qh_REAL_1 "\n", id, id2, facet->id, dist);
    }
    break;
  case qh_PRINTpoints:  /* VORONOI only by qh_printbegin */
    if (qh CDDoutput)
      gInterface->HullPrintf (fp, "1 ");
    qh_printcenter (fp, format, NULL, facet);
    break;
  case qh_PRINTvertices:
    gInterface->HullPrintf (fp, "%d", qh_setsize (facet->vertices));
    FOREACHvertex_(facet->vertices)
      gInterface->HullPrintf (fp, " %d", qh_pointid (vertex->point));
    gInterface->HullPrintf (fp, "\n");
    break;
  }
} /* printafacet */
...
void qh_printbegin (FILE *fp, int format, facetT *facetlist, setT *facets, boolT printall) 
{
  ...
  qh printoutnum= 0;
  qh_countfacets (facetlist, facets, printall, &numfacets, &numsimplicial, &totneighbors, &numridges, &numcoplanars, &numtricoplanars);
  ...
  switch (format)
  {
    ...
    case qh_PRINTfacets:
    case qh_PRINTfacets_xridge:
      if (facetlist) qh_printvertexlist (fp, "Vertices and facets:\n", facetlist, facets, printall);
      break;
    case qh_PRINTgeom:
      ...
      if (qh hull_dim == 2 && (qh PRINTridges || qh DOintersections))
        gInterface->HullPrintf (qh ferr, "qhull warning: output for ridges and intersections not implemented in 2-d\n");
      if (qh hull_dim == 4 && (qh PRINTinner || qh PRINTouter || (qh PRINTdim == 4 && qh PRINTcentrums)))
        gInterface->HullPrintf (qh ferr, "qhull warning: output for outer/inner planes and centrums not implemented in 4-d\n");
      if (qh PRINTdim == 4 && (qh PRINTspheres))
        gInterface->HullPrintf (qh ferr, "qhull warning: output for vertices not implemented in 4-d\n");
      if (qh PRINTdim == 4 && qh DOintersections && qh PRINTnoplanes)
        gInterface->HullPrintf (qh ferr, "qhull warning: 'Gnh' generates no output in 4-d\n");
      if (qh PRINTdim == 2)
      {
        gInterface->HullPrintf(fp, "{appearance {linewidth 3} LIST # %s | %s\n", qh rbox_command, qh qhull_command);
      }
      else if (qh PRINTdim == 3)
      {
        gInterface->HullPrintf(fp, "{appearance {+edge -evert linewidth 2} LIST # %s | %s\n", qh rbox_command, qh qhull_command);
      }
      else if (qh PRINTdim == 4)
      {
        ...
        gInterface->HullPrintf (fp, "LIST # %s | %s\n", qh rbox_command, qh qhull_command);
      }
      ...
      if (qh PRINTdots)
      {
        ...
        if (qh PRINTdim == 4)
          gInterface->HullPrintf (fp, "4VECT %d %d 1\n", num, num);
        else
        	gInterface->HullPrintf (fp, "VECT %d %d 1\n", num, num);

        for (i= num; i--; )
        {
          if (i % 20 == 0)  gInterface->HullPrintf (fp, "\n");
        	gInterface->HullPrintf (fp, "1 ");
        }
        gInterface->HullPrintf (fp, "# 1 point per line\n1 ");
        for (i= num-1; i--; )
        {
          if (i % 20 == 0) gInterface->HullPrintf (fp, "\n");
      	  gInterface->HullPrintf (fp, "0 ");
        }
        gInterface->HullPrintf (fp, "# 1 color for all\n");
        FORALLpoints
        {
        ...
        gInterface->HullPrintf (fp, "0 1 1 1  # color of points\n");
      }

      if (qh PRINTdim == 4  && !qh PRINTnoplanes)  /* 4dview loads up multiple 4OFF objects slowly */
        gInterface->HullPrintf(fp, "4OFF %d %d 1\n", 3*qh ridgeoutnum, qh ridgeoutnum);
      ...
      if (qh PRINTspheres + qh PRINTcoplanar)    maximize_(qh PRINTradius, qh MAXabs_coord * qh_MINradius);
      ...
      if (qh JOGGLEmax < REALmax/2)      qh PRINTradius += qh JOGGLEmax * (double)sqrt ((double)qh hull_dim);
      if (qh PRINTdim != 4 &&	(qh PRINTcoplanar || qh PRINTspheres || qh PRINTcentrums))
      {
        vertices= qh_facetvertices (facetlist, facets, printall);
        if (qh PRINTspheres && qh PRINTdim <= 3)         qh_printspheres (fp, vertices, qh PRINTradius);
        if (qh PRINTcoplanar || qh PRINTcentrums)
        {
          ...
          FORALLfacet_(facetlist)
          {
            ...
            if (qh PRINTcentrums && qh PRINTdim <= 3)  qh_printcentrum (fp, facet, qh PRINTcradius);
    ...
    case qh_PRINTincidences:
      if (qh VORONOI && qh PRINTprecision)
        gInterface->HullPrintf (qh ferr, "qhull warning: writing Delaunay.  Use 'p' or 'o' for Voronoi centers\n");
    ...
    case qh_PRINTouter:
      if (qh CDDoutput)
        gInterface->HullPrintf (fp, "%s | %s\nbegin\n    %d %d real\n", qh rbox_command,  qh qhull_command, numfacets, qh hull_dim+1);
      else
    ...
    case qh_PRINTmaple:
      if (qh hull_dim > 3)  /* qh_initbuffers also checks */
        goto LABELnoformat;
      if (qh VORONOI)       gInterface->HullPrintf (qh ferr, "qhull warning: output is the Delaunay triangulation\n");
      if (format == qh_PRINTmaple)
      {
        if (qh hull_dim == 2)
	        gInterface->HullPrintf(fp, "PLOT(CURVES(\n");
        else
	        gInterface->HullPrintf(fp, "PLOT3D(POLYGONS(\n");
      }
      else
        gInterface->HullPrintf(fp, "{\n");
    ...
    case qh_PRINTtriangles:
      ...
      {
        gInterface->HullBegin( num,  qh num_points+qh_setsize (qh other_points), numfacets, totneighbors/2 );
        //gInterface->HullPrintf (fp, "%d\n%d %d %d\n", num, qh num_points+qh_setsize (qh other_points), numfacets, totneighbors/2);
      }
      else
      { /* qh_PRINTtriangles */
        qh printoutvar= qh num_points+qh_setsize (qh other_points); /* first centrum */
        if (qh DELAUNAY) num--;  /* drop last dimension */
        gInterface->HullPrintf (fp, "%d\n%d %d %d\n", num, qh printoutvar	+ numfacets - numsimplicial, numsimplicial + numridges, totneighbors/2);
      }
    ...
    case qh_PRINTpoints:
      if (!qh VORONOI)
        goto LABELnoformat;
      if (qh CDDoutput)
        gInterface->HullPrintf (fp, "%s | %s\nbegin\n%d %d real\n", qh rbox_command,qh qhull_command, numfacets, qh hull_dim);
      else
        gInterface->HullPrintf (fp, "%d\n%d\n", qh hull_dim-1, numfacets);
      break;
    case qh_PRINTvertices:
    ...
    case qh_PRINTsummary:
      default:
    LABELnoformat:
      gInterface->HullPrintf (qh ferr, "qhull internal error (qh_printbegin): can not use this format for dimension %d\n", qh hull_dim);
      qh_errexit (qh_ERRqhull, NULL, NULL);
  }
} /* printbegin */
...
void qh_printcenter (FILE *fp, int format, const char *string, facetT *facet) {
  ...
  if (qh CENTERtype != qh_ASvoronoi && qh CENTERtype != qh_AScentrum)
    return;
  if (string)
    gInterface->HullPrintf (fp, string, facet->id);
  if (qh CENTERtype == qh_ASvoronoi) {
    ...
    if (!facet->normal || !facet->upperdelaunay || !qh ATinfinity) {
      if (!facet->center)
        facet->center= qh_facetcenter (facet->vertices);
      for (k=0; k < num; k++)
        gInterface->HullPrintf (fp, qh_REAL_1, facet->center[k]);
    }else {
      for (k=0; k < num; k++)
        gInterface->HullPrintf (fp, qh_REAL_1, qh_INFINITE);
    }
...
void qh_printcentrum (FILE *fp, facetT *facet, realT radius) {
  ...
  gInterface->HullPrintf (fp, "{appearance {-normal -edge normscale 0} ");
  if (qh firstcentrum) {
    qh firstcentrum= False;
    gInterface->HullPrintf (fp, "{INST geom { define centrum CQUAD  # f%d\n\
-0.3 -0.3 0.0001     0 0 1 1\n\
 0.3 -0.3 0.0001     0 0 1 1\n\
 0.3  0.3 0.0001     0 0 1 1\n\
-0.3  0.3 0.0001     0 0 1 1 } transform { \n", facet->id);
  }else
    gInterface->HullPrintf (fp, "{INST geom { : centrum } transform { # f%d\n", facet->id);
  ...
  gInterface->HullPrintf (fp, "%8.4g %8.4g %8.4g 0\n", xaxis[0], xaxis[1], xaxis[2]);
...
void qh_printend (FILE *fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (!qh printoutnum)
    gInterface->HullPrintf (qh ferr, "qhull warning: no facets printed\n");
  switch (format) {
  case qh_PRINTgeom:
	...
	gInterface->HullPrintf (qh ferr, "qhull internal error (qh_printend): number of ridges %d != number printed %d and at end %d\n", qh ridgeoutnum, qh printoutvar, num);
	qh_errexit (qh_ERRqhull, NULL, NULL);
      }
    }else
      gInterface->HullPrintf(fp, "}\n");
...
void qh_printend4geom (FILE *fp, facetT *facet, int *nump, boolT printall) {
  ...
  if (facet->simplicial) {
	...
	if (fp)
          gInterface->HullPrintf (fp, "3 %d %d %d %8.4g %8.4g %8.4g 1 # f%d f%d\n",
    ...
    FOREACHridge_(facet->ridges) {
      ...
      if (neighbor->visitid != qh visit_id) {
	if (fp)
          gInterface->HullPrintf (fp, "3 %d %d %d %8.4g %8.4g %8.4g 1 #r%d f%d f%d\n",
		 3*num, 3*num+1, 3*num+2, color[0], color[1], color[2],
...
void qh_printextremes_2d (FILE *fp, facetT *facetlist, setT *facets, int printall) {
  ...
  do {
    ...
    if (facet->visitid == qh visit_id) {
      gInterface->HullPrintf(qh ferr, "qh_printextremes_2d: loop in facet list.  facet %d nextfacet %d\n",
...
void qh_printfacet2math(FILE *fp, facetT *facet, int format, int notfirst) {
  ...
  gInterface->HullPrintf(fp, pointfmt, point0[0], point0[1], point1[0], point1[1]);
  qh_memfree (point1, qh normal_size);
  qh_memfree (point0, qh normal_size);
} /* printfacet2math */
...
void qh_printfacet3geom_points(FILE *fp, setT *points, facetT *facet, realT offset, realT color[3]) {
  ...
  gInterface->HullPrintf(fp, "{ OFF %d 1 1 # f%d\n", n, facet->id);
  if (offset != 0.0) {
  ...
  FOREACHpoint_(printpoints) {
    for (k=0; k < qh hull_dim; k++) {
      if (k == qh DROPdim)
        ...
        gInterface->HullPrintf(fp, "%8.4g ", point[k]);
    }
  ...
  gInterface->HullPrintf(fp, "%d ", n);
  for(i= 0; i < n; i++)
    gInterface->HullPrintf(fp, "%d ", i);
  gInterface->HullPrintf(fp, "%8.4g %8.4g %8.4g 1.0 }\n", color[0], color[1], color[2]);
...
void qh_printfacet3math (FILE *fp, facetT *facet, int format, int notfirst) {
  ...
  if (format == qh_PRINTmaple) {
    ...
    gInterface->HullPrintf(fp, "Polygon[{");
    pointfmt= "{%16.8f, %16.8f, %16.8f}";
    endfmt= "}]";
  }
  FOREACHpoint_(points) {
    if (firstpoint)
      ...
      gInterface->HullPrintf(fp, ",\n");
    gInterface->HullPrintf(fp, pointfmt, point[0], point[1], point[2]);
  }
  FOREACHpoint_(points)
    qh_memfree (point, qh normal_size);
  qh_settempfree(&points);
  qh_settempfree(&vertices);
  gInterface->HullPrintf(fp, endfmt);
} /* printfacet3math */
...
void qh_printfacet3vertex(FILE * /*fp*/, facetT *facet, int /*format*/) 
{
  ...
  #define MAXFACE 256
  unsigned int polygon[MAXFACE];
  unsigned int fcount =  qh_setsize (vertices);
  assert( fcount < MAXFACE );

  unsigned int index=0;

  FOREACHvertex_(vertices) 
  {
    polygon[index] =qh_pointid(vertex->point);
    index++;
  }

  gInterface->HullPolygon(fcount, polygon );

//if (format == qh_PRINToff)
//    gInterface->HullPrintf (fp, "%d ", qh_setsize (vertices));
//
//  FOREACHvertex_(vertices) 
//    gInterface->HullPrintf (fp, "%d ", qh_pointid(vertex->point));
//  gInterface->HullPrintf (fp, "\n");
...
void qh_printfacet4geom_nonsimplicial(FILE *fp, facetT *facet, realT color[3]) {
  ...
  FOREACHridge_(facet->ridges) {
    ...
    if (qh DOintersections)
      ...
      if (qh DROPdim >= 0) 
	gInterface->HullPrintf(fp, "OFF 3 1 1 # f%d\n", facet->id);
      else {
	qh printoutvar++;
	gInterface->HullPrintf (fp, "# r%d between f%d f%d\n", ridge->id, facet->id, neighbor->id);
      }
	...
	qh_memfree (point, qh normal_size);
      ...
      if (qh DROPdim >= 0)
        gInterface->HullPrintf(fp, "3 0 1 2 %8.4g %8.4g %8.4g\n", color[0], color[1], color[2]);
    }
  }
...
void qh_printfacet4geom_simplicial(FILE *fp, facetT *facet, realT color[3]) {
  ...
  FOREACHneighbor_(facet) {
    ...
    if (qh DOintersections)
      ...
      if (qh DROPdim >= 0) 
	gInterface->HullPrintf(fp, "OFF 3 1 1 # ridge between f%d f%d\n",
		facet->id, neighbor->id);
      else {
	qh printoutvar++;
	gInterface->HullPrintf (fp, "# ridge between f%d f%d\n", facet->id, neighbor->id);
      }
      FOREACHvertex_(vertices) {
	for(k= 0; k < qh hull_dim; k++) {
	  if (k != qh DROPdim)
  	    gInterface->HullPrintf(fp, "%8.4g ", vertex->point[k]);
  	}
	gInterface->HullPrintf (fp, "\n");
      }
      if (qh DROPdim >= 0) 
...
void qh_printfacetNvertex_nonsimplicial(FILE *fp, facetT *facet, int id, int format) {
  ...
  FOREACHridge_(facet->ridges) {
    if (format == qh_PRINTtriangles)
      gInterface->HullPrintf(fp, "%d ", qh hull_dim);
    gInterface->HullPrintf(fp, "%d ", id);
...
void qh_printfacetheader(FILE *fp, facetT *facet) {
  ...
  if (facet == qh_MERGEridge) {
    gInterface->HullPrintf (fp, " MERGEridge\n");
    return;
  }else if (facet == qh_DUPLICATEridge) {
    gInterface->HullPrintf (fp, " DUPLICATEridge\n");
    return;
  }else if (!facet) {
    gInterface->HullPrintf (fp, " NULLfacet\n");
    return;
  }
  qh old_randomdist= qh RANDOMdist;
  qh RANDOMdist= False;
  gInterface->HullPrintf(fp, "- f%d\n", facet->id);
  gInterface->HullPrintf(fp, "    - flags:");
  if (facet->toporient) 
    gInterface->HullPrintf(fp, " top");
  else
    gInterface->HullPrintf(fp, " bottom");
  if (facet->simplicial)
    gInterface->HullPrintf(fp, " simplicial");
  if (facet->tricoplanar)
    gInterface->HullPrintf(fp, " tricoplanar");
  if (facet->upperdelaunay)
    gInterface->HullPrintf(fp, " upperDelaunay");
  if (facet->visible)
    gInterface->HullPrintf(fp, " visible");
  if (facet->newfacet)
    gInterface->HullPrintf(fp, " new");
  if (facet->tested)
    gInterface->HullPrintf(fp, " tested");
  if (!facet->good)
    gInterface->HullPrintf(fp, " notG");
  if (facet->seen)
    gInterface->HullPrintf(fp, " seen");
  if (facet->coplanar)
    gInterface->HullPrintf(fp, " coplanar");
  if (facet->mergehorizon)
    gInterface->HullPrintf(fp, " mergehorizon");
  if (facet->keepcentrum)
    gInterface->HullPrintf(fp, " keepcentrum");
  if (facet->dupridge)
    gInterface->HullPrintf(fp, " dupridge");
  if (facet->mergeridge && !facet->mergeridge2)
    gInterface->HullPrintf(fp, " mergeridge1");
  if (facet->mergeridge2)
    gInterface->HullPrintf(fp, " mergeridge2");
  if (facet->newmerge)
    gInterface->HullPrintf(fp, " newmerge");
  if (facet->flipped) 
    gInterface->HullPrintf(fp, " flipped");
  if (facet->notfurthest) 
    gInterface->HullPrintf(fp, " notfurthest");
  if (facet->degenerate)
    gInterface->HullPrintf(fp, " degenerate");
  if (facet->redundant)
    gInterface->HullPrintf(fp, " redundant");
  gInterface->HullPrintf(fp, "\n");
  if (facet->isarea)
    gInterface->HullPrintf(fp, "    - area: %2.2g\n", facet->f.area);
  else if (qh NEWfacets && facet->visible && facet->f.replace)
    gInterface->HullPrintf(fp, "    - replacement: f%d\n", facet->f.replace->id);
  else if (facet->newfacet) {
    if (facet->f.samecycle && facet->f.samecycle != facet)
      gInterface->HullPrintf(fp, "    - shares same visible/horizon as f%d\n", facet->f.samecycle->id);
  }else if (facet->tricoplanar /* !isarea */) {
    if (facet->f.triowner)
      gInterface->HullPrintf(fp, "    - owner of normal & centrum is facet f%d\n", facet->f.triowner->id);
  }else if (facet->f.newcycle)
    gInterface->HullPrintf(fp, "    - was horizon to f%d\n", facet->f.newcycle->id);
  if (facet->nummerge)
    gInterface->HullPrintf(fp, "    - merges: %d\n", facet->nummerge);
  qh_printpointid(fp, "    - normal: ", qh hull_dim, facet->normal, -1);
  gInterface->HullPrintf(fp, "    - offset: %10.7g\n", facet->offset);
  if (qh CENTERtype == qh_ASvoronoi || facet->center)
    qh_printcenter (fp, qh_PRINTfacets, "    - center: ", facet);
#if qh_MAXoutside
  if (facet->maxoutside > qh DISTround)
    gInterface->HullPrintf(fp, "    - maxoutside: %10.7g\n", facet->maxoutside);
#endif
  if (!SETempty_(facet->outsideset)) {
    furthest= (pointT*)qh_setlast(facet->outsideset);
    if (qh_setsize (facet->outsideset) < 6) {
      gInterface->HullPrintf(fp, "    - outside set (furthest p%d):\n", qh_pointid(furthest));
      FOREACHpoint_(facet->outsideset)
	qh_printpoint(fp, "     ", point);
    }else if (qh_setsize (facet->outsideset) < 21) {
      qh_printpoints(fp, "    - outside set:", facet->outsideset);
    }else {
      gInterface->HullPrintf(fp, "    - outside set:  %d points.", qh_setsize(facet->outsideset));
      qh_printpoint(fp, "  Furthest", furthest);
    }
#if !qh_COMPUTEfurthest
    gInterface->HullPrintf(fp, "    - furthest distance= %2.2g\n", facet->furthestdist);
#endif
  }
  if (!SETempty_(facet->coplanarset)) {
    furthest= (pointT*)qh_setlast(facet->coplanarset);
    if (qh_setsize (facet->coplanarset) < 6) {
      gInterface->HullPrintf(fp, "    - coplanar set (furthest p%d):\n", qh_pointid(furthest));
      FOREACHpoint_(facet->coplanarset)
	qh_printpoint(fp, "     ", point);
    }else if (qh_setsize (facet->coplanarset) < 21) {
      qh_printpoints(fp, "    - coplanar set:", facet->coplanarset);
    }else {
      gInterface->HullPrintf(fp, "    - coplanar set:  %d points.", qh_setsize(facet->coplanarset));
      qh_printpoint(fp, "  Furthest", furthest);
    }
    zinc_(Zdistio);
  ...
  gInterface->HullPrintf(fp, "    - neighboring facets: ");
  FOREACHneighbor_(facet) {
    if (neighbor == qh_MERGEridge)
      gInterface->HullPrintf(fp, " MERGE");
    else if (neighbor == qh_DUPLICATEridge)
      gInterface->HullPrintf(fp, " DUP");
    else
      gInterface->HullPrintf(fp, " f%d", neighbor->id);
  }
  gInterface->HullPrintf(fp, "\n");
...
void qh_printfacetridges(FILE *fp, facetT *facet) {
  ...
  if (facet->visible && qh NEWfacets) {
    gInterface->HullPrintf(fp, "    - ridges (ids may be garbage):");
    FOREACHridge_(facet->ridges)
      gInterface->HullPrintf(fp, " r%d", ridge->id);
    gInterface->HullPrintf(fp, "\n");
  }else {
    gInterface->HullPrintf(fp, "    - ridges:\n");
    FOREACHridge_(facet->ridges)
      ridge->seen= False;
    if (qh hull_dim == 3) {
    ...
    if (numridges != qh_setsize (facet->ridges)) {
      gInterface->HullPrintf (fp, "     - all ridges:");
      FOREACHridge_(facet->ridges) 
	gInterface->HullPrintf (fp, " r%d", ridge->id);
        gInterface->HullPrintf (fp, "\n");
    }
...
void qh_printfacets(FILE *fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (qh CDDoutput && (format == qh_PRINTcentrums || format == qh_PRINTpointintersect || format == qh_PRINToff))
    gInterface->HullPrintf (qh ferr, "qhull warning: CDD format is not available for centrums, halfspace\nintersections, and OFF file format.\n");
  if (format == qh_PRINTnone)
    ; /* print nothing */
  else if (format == qh_PRINTaverage) {
    vertices= qh_facetvertices (facetlist, facets, printall);
    center= qh_getcenter (vertices);
    gInterface->HullPrintf (fp, "%d 1\n", qh hull_dim);
    ...
    gInterface->HullPrintf(fp, "Options selected for Qhull %s:\n%s\n", qh_version, qh qhull_options);
  ...
  {
    gInterface->HullAreaVolume( qh totarea , qh totvol );
  }else if (format == qh_PRINTsummary) {
    qh_countfacets (facetlist, facets, printall, &numfacets, &numsimplicial, 
      &totneighbors, &numridges, &numcoplanars, &numtricoplanars);
    vertices= qh_facetvertices (facetlist, facets, printall); 
    gInterface->HullPrintf (fp, "10 %d %d %d %d %d %d %d %d %d %d\n2 ", qh hull_dim, 
    ...
    gInterface->HullPrintf (fp, qh_REAL_2n, outerplane, innerplane);
...
to the center point are as follows:\n\n", qh DISTround);
  ...
  gInterface->HullPrintf (fp, "\n");
  FORALLfacets {
    gInterface->HullPrintf (fp, "facet");
    FOREACHvertex_(facet->vertices)
      gInterface->HullPrintf (fp, " p%d", qh_pointid(vertex->point));
    zinc_(Zdistio);
    qh_distplane(qh interior_point, facet, &dist);
    gInterface->HullPrintf (fp, " distance= %4.2g\n", dist);
  }
  if (!qh_QUICKhelp) {
    if (qh HALFspace) 
      gInterface->HullPrintf (fp, "\n\
...
the points by randomly rotating the input with 'QR0'.\n");
  ...
  for (k=0; k < qh hull_dim; k++) {
    ...
    gInterface->HullPrintf (fp, "  %d:  %8.4g  %8.4g  difference= %4.4g\n", k, min, max, max-min);
  }
  if (!qh_QUICKhelp) {
    gInterface->HullPrintf (fp, "\n\
...
void qh_printhyperplaneintersection(FILE *fp, facetT *facet1, facetT *facet2,
  ...
  if (qh hull_dim == 3)
    gInterface->HullPrintf(fp, "VECT 1 %d 1 %d 1 ", i, i);
  else if (qh hull_dim == 4 && qh DROPdim >= 0)
    gInterface->HullPrintf(fp, "OFF 3 1 1 ");
  else
    qh printoutvar++;
  gInterface->HullPrintf (fp, "# intersect f%d f%d\n", facet1->id, facet2->id);
  ...
  FOREACHvertex_(vertices) {
    ...
    if (qh PRINTdim <= 3) {
      qh_projectdim3 (p, p);
      gInterface->HullPrintf(fp, "%8.4g %8.4g %8.4g # ", p[0], p[1], p[2]);
    }else 
      gInterface->HullPrintf(fp, "%8.4g %8.4g %8.4g %8.4g # ", p[0], p[1], p[2], p[3]);
    if (nearzero1+nearzero2)
      gInterface->HullPrintf (fp, "p%d (coplanar facets)\n", qh_pointid (vertex->point));
    else
      gInterface->HullPrintf (fp, "projected p%d\n", qh_pointid (vertex->point));
  }
  if (qh hull_dim == 3)
    gInterface->HullPrintf(fp, "%8.4g %8.4g %8.4g 1.0\n", color[0], color[1], color[2]); 
  else if (qh hull_dim == 4 && qh DROPdim >= 0)  
    gInterface->HullPrintf(fp, "3 0 1 2 %8.4g %8.4g %8.4g 1.0\n", color[0], color[1], color[2]);
...
void qh_printline3geom (FILE *fp, pointT *pointA, pointT *pointB, realT color[3]) {
  ...
  if ((fabs(pA[0] - pB[0]) > 1e-3) || 
      (fabs(pA[1] - pB[1]) > 1e-3) || 
      (fabs(pA[2] - pB[2]) > 1e-3)) {
    gInterface->HullPrintf (fp, "VECT 1 2 1 2 1\n");
    for (k= 0; k < 3; k++)
       gInterface->HullPrintf (fp, "%8.4g ", pB[k]);
    gInterface->HullPrintf (fp, " # p%d\n", qh_pointid (pointB));
  }else
    gInterface->HullPrintf (fp, "VECT 1 1 1 1 1\n");
  for (k=0; k < 3; k++)
    gInterface->HullPrintf (fp, "%8.4g ", pA[k]);
  gInterface->HullPrintf (fp, " # p%d\n", qh_pointid (pointA));
  gInterface->HullPrintf (fp, "%8.4g %8.4g %8.4g 1\n", color[0], color[1], color[2]);
}
...
void qh_printpointid(FILE *fp, const char *string, int dim, pointT *point, int id) 
{
  ...
  if (string) 
  {
    fputs (string, fp);
    if (id != -1)
      gInterface->HullPrintf(fp, " p%d: ", id);
  }

  if ( dim == 3 )
  {
    gInterface->HullVertex( (float)point[0], (float)point[1], (float)point[2] );
  }
  else
  {
    assert(0); // I only support 3d convex hulls!
  }
...
void qh_printpoint3 (FILE *fp, pointT *point) {
  ...
  for (k=0; k < 3; k++)
    gInterface->HullPrintf (fp, "%8.4g ", p[k]);
  gInterface->HullPrintf (fp, " # p%d\n", qh_pointid (point));
} /* printpoint3 */
...
void qh_printridge(FILE *fp, ridgeT *ridge) {
  ...
  if (ridge->tested)
    gInterface->HullPrintf (fp, " tested");
  if (ridge->nonconvex)
    gInterface->HullPrintf (fp, " nonconvex");
  gInterface->HullPrintf (fp, "\n");
  qh_printvertices (fp, "           vertices:", ridge->vertices);
  if (ridge->top && ridge->bottom)
    gInterface->HullPrintf(fp, "           between f%d and f%d\n",
...
void qh_printspheres(FILE *fp, setT *vertices, realT radius) {
  ...
  qh printoutnum++;
  gInterface->HullPrintf (fp, "{appearance {-edge -normal normscale 0} {\n\
...
3 17 10 16\n} transforms { TLIST\n");
  FOREACHvertex_(vertices) {
    gInterface->HullPrintf(fp, "%8.4g 0 0 0 # v%d\n 0 %8.4g 0 0\n0 0 %8.4g 0\n",
...
void qh_printvdiagram (FILE *fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (format == qh_PRINTvertices) {
    ...
    gInterface->HullPrintf(qh ferr, "qh_printvdiagram: unknown print format %d.\n", format);
    qh_errexit (qh_ERRinput, NULL, NULL);
  }
...
void qh_printvertex(FILE *fp, vertexT *vertex) {
  ...
  if (!vertex) {
    gInterface->HullPrintf (fp, "  NULLvertex\n");
    return;
  }
  gInterface->HullPrintf(fp, "- p%d (v%d):", qh_pointid(vertex->point), vertex->id);
  point= vertex->point;
  if (point) {
    for(k= qh hull_dim; k--; ) {
      r= *point++;
      gInterface->HullPrintf(fp, " %5.2g", r);
    }
  }
  if (vertex->deleted)
    gInterface->HullPrintf(fp, " deleted");
  if (vertex->delridge)
    gInterface->HullPrintf (fp, " ridgedeleted");
  gInterface->HullPrintf(fp, "\n");
  if (vertex->neighbors) {
    gInterface->HullPrintf(fp, "  neighbors:");
    FOREACHneighbor_(vertex) {
      if (++count % 100 == 0)
	gInterface->HullPrintf (fp, "\n     ");
      gInterface->HullPrintf(fp, " f%d", neighbor->id);
...
void qh_printvneighbors (FILE *fp, facetT* facetlist, setT *facets, boolT printall) {
  ...
  FOREACHvertex_i_(vertex_points) {
    if (vertex) { 
      ...
      gInterface->HullPrintf (fp, "%d", numneighbors);
      if (qh hull_dim == 3)
        qh_order_vertexneighbors (vertex);
      else if (qh hull_dim >= 4)
...
void qh_printvoronoi (FILE *fp, int format, facetT *facetlist, setT *facets, boolT printall) {
  ...
  if (format == qh_PRINTgeom) 
    gInterface->HullPrintf (fp, "{appearance {+edge -face} OFF %d %d 1 # Voronoi centers and cells\n", 
                numcenters, numvertices);
  else
    gInterface->HullPrintf (fp, "%d\n%d %d 1\n", qh hull_dim-1, numcenters, qh_setsize(vertices));
  if (format == qh_PRINTgeom) {
    for (k= qh hull_dim-1; k--; )
      gInterface->HullPrintf (fp, qh_REAL_1, 0.0);
    gInterface->HullPrintf (fp, " 0 # infinity not used\n");
  }else {
    for (k= qh hull_dim-1; k--; )
      gInterface->HullPrintf (fp, qh_REAL_1, qh_INFINITE);
    gInterface->HullPrintf (fp, "\n");
  ...
  FORALLfacet_(facetlist) {
    if (facet->visitid && facet->visitid < numfacets) {
      if (format == qh_PRINTgeom)
        gInterface->HullPrintf (fp, "# %d f%d\n", vid++, facet->id);
      qh_printcenter (fp, format, NULL, facet);
    }
  }
  FOREACHfacet_(facets) {
	...
	if (vertex) {
	  FOREACHneighbor_(vertex) {
	    if (neighbor->visitid && neighbor->visitid < numfacets)
	      gInterface->HullPrintf (fp, " %d", neighbor->visitid);
	  }
	}
	gInterface->HullPrintf (fp, " # p%d (v%d)\n", vertex_i, vertex->id);
      }else
	gInterface->HullPrintf (fp, " # p%d is coplanar or isolated\n", vertex_i);
    }else {
      if (numinf)
	numneighbors++;
      gInterface->HullPrintf (fp, "%d", numneighbors);
...
void qh_printvnorm (FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers, boolT /*unbounded*/) {
  ...
  gInterface->HullPrintf (fp, "%d %d %d ", 
      2+qh hull_dim, qh_pointid (vertex->point), qh_pointid (vertexA->point));
  for (k= 0; k< qh hull_dim-1; k++)
    gInterface->HullPrintf (fp, qh_REAL_1, normal[k]);
  gInterface->HullPrintf (fp, qh_REAL_1, offset);
  gInterface->HullPrintf (fp, "\n");
} /* printvnorm */
...
void qh_printvridge (FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers, boolT /*unbounded*/) {
  ...
  gInterface->HullPrintf (fp, "%d %d %d", qh_setsize (centers)+2, 
       qh_pointid (vertex->point), qh_pointid (vertexA->point));
  FOREACHfacet_(centers) 
    gInterface->HullPrintf (fp, " %d", facet->visitid);
  gInterface->HullPrintf (fp, "\n");
} /* printvridge */
...
int qh_readfeasible (int dim, char *remainder) {
  ...
  if (!qh HALFspace) {
    gInterface->HullPrintf  (qh ferr, "qhull input error: feasible point (dim 1 coords) is only valid for halfspace intersection\n");
    qh_errexit (qh_ERRinput, NULL, NULL);
  }  
  if (qh feasible_string)
    gInterface->HullPrintf  (qh ferr, "qhull input warning: feasible point (dim 1 coords) overrides 'Hn,n,n' feasible point for halfspace intersection\n");
  if (!(qh feasible_point= (coordT*)malloc (dim* sizeof(coordT)))) {
    gInterface->HullPrintf(qh ferr, "qhull error: insufficient memory for feasible point\n");
    ...
    while (*s) {
      ...
      *(coords++)= value;
      if (++tokcount == dim) {
        ...
        qh_strtod (s, &t);
        if (s != t) {
          gInterface->HullPrintf (qh ferr, "qhull input error: coordinates for feasible point do not finish out the line: %s\n",
               s);
          qh_errexit (qh_ERRinput, NULL, NULL);
        }
        return linecount;
  ...
  gInterface->HullPrintf (qh ferr, "qhull input error: only %d coordinates.  Could not read %d-d feasible point.\n",
           tokcount, dim);
  qh_errexit (qh_ERRinput, NULL, NULL);
  return 0;
} /* readfeasible */
...
coordT *qh_readpoints(int *numpoints, int *dimension, boolT *ismalloc) {
  ...
  if (qh CDDinput) {
    ...
    if (!s) {
      gInterface->HullPrintf (qh ferr, "qhull input error: missing \"begin\" for cdd-formated input\n");
      qh_errexit (qh_ERRinput, NULL, NULL);
    }
  }
  ...
  if (!s) {
    gInterface->HullPrintf(qh ferr, "qhull input error: short input file.  Did not find dimension and number of points\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  ...
  if (diminput < 2) {
    gInterface->HullPrintf(qh ferr,"qhull input error: dimension %d (first number) should be at least 2\n",
	    diminput);
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  if (isdelaunay) {
    ...
    *numpoints= numinput;
    if (diminput < 3) {
      gInterface->HullPrintf(qh ferr,"qhull input error: dimension %d (first number, includes offset) should be at least 3 for halfspaces\n",
  	    diminput);
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
    if (dimfeasible) {
      if (dimfeasible != *dimension) {
        gInterface->HullPrintf(qh ferr,"qhull input error: dimension %d of feasible point is not one less than dimension %d for halfspaces\n",
          dimfeasible, diminput);
        qh_errexit(qh_ERRinput, NULL, NULL);
      }
    }else 
  ...
  *ismalloc= True;  /* use malloc since memory not setup */
  ...
  if (!coords || !qh line || (qh HALFspace && !qh half_space)) {
    gInterface->HullPrintf(qh ferr, "qhull error: insufficient memory to read %d points\n",
	    numinput);
    qh_errexit(qh_ERRmem, NULL, NULL);
  }
  ...
  while ((s= (isfirst ?  s : fgets(qh line, qh maxline, qh fin)))) {
	...
	if (!memcmp (s, "end", 3) || !memcmp (s, "END", 3)) {
	  if (qh CDDinput )
	    ...
	    gInterface->HullPrintf (qh ferr, "qhull input warning: the input appears to be in cdd format.  If so, use 'Fd'\n");
	}
      }
    }
    islong= False;
	  ...
	  *(coordp++)= value;
        ...
        *(coords++)= value;
        if (qh CDDinput && !coordcount) {
          if (value != 1.0) {
            gInterface->HullPrintf (qh ferr, "qhull input error: for cdd format, point at line %d does not start with '1'\n",
                   linecount);
            qh_errexit (qh_ERRinput, NULL, NULL);
          }
          coords--;
      ...
      if (++coordcount == diminput) {
	    ...
	    gInterface->HullPrintf (qh ferr, "The halfspace was on line %d\n", linecount);
	    if (wasbegin)
	      gInterface->HullPrintf (qh ferr, "The input appears to be in cdd format.  If so, you should use option 'Fd'\n");
	    qh_errexit (qh_ERRinput, NULL, NULL);
	  }
    ...
    if (!isfirst && s - qh line >= qh maxline) {
      gInterface->HullPrintf(qh ferr, "qhull input error: line %d contained more than %d characters\n", 
...
%d points and %d extra coordinates.  Line %d is the first\npoint",
    ...
    if (firsttext)
      gInterface->HullPrintf(qh ferr, ", line %d is the first comment", firsttext);
    if (firstshort)
      gInterface->HullPrintf(qh ferr, ", line %d is the first short\nline", firstshort);
    if (firstlong)
      gInterface->HullPrintf(qh ferr, ", line %d is the first long line", firstlong);
    gInterface->HullPrintf(qh ferr, ".  Continue with %d points.\n", newnum);
    numinput= newnum;
    if (isdelaunay && qh ATinfinity) {
...
Either prepend the input with 1 point or use 'Hn,n,n'.  See manual.\n");
  ...
  if (!(qh feasible_point= (pointT*)malloc (dim* sizeof(coordT)))) {
    gInterface->HullPrintf(qh ferr, "qhull error: insufficient memory for 'Hn,n,n'\n");
    qh_errexit(qh_ERRmem, NULL, NULL);
  }
  coords= qh feasible_point;
  while (*s) {
    value= qh_strtod (s, &s);
    if (++tokcount > dim) {
      gInterface->HullPrintf (qh ferr, "qhull input warning: more coordinates for 'H%s' than dimension %d\n",
