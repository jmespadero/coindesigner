...
void qh_setaddnth(setT **setp, int nth, void *newelem) {
  ...
  {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_setaddnth): nth %d is out-of-bounds for set:\n", nth);
...
void qh_setcheck(setT *set, const char *tname, int id)
{
  ...
  {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_setcheck): actual size %d of %s%d is greater than max size %d\n", size, tname, id, maxsize);
    waserr= 1;
  }else if (set->e[size].p)
  {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_setcheck): %s%d (size %d max %d) is not null terminated.\n", tname, id, maxsize, size-1);
    waserr= 1;
  }
  if (waserr) {
...
void qh_setprint(FILE *fp, const char* string, setT *set) {
  ...
  if (!set)
  {
    gInterface->HullPrintf (fp, "%s set is null\n", string);
  }
  else
  {
    SETreturnsize_(set, size);
    gInterface->HullPrintf (fp, "%s set=%p maxsize=%d size=%d elems=",string, set, set->maxsize, size);
    if (size > set->maxsize)
      size= set->maxsize+1;
    for (k=0; k < size; k++)
    {
      gInterface->HullPrintf(fp, " %p", set->e[k].p);
    }
...
void qh_setreplace(setT *set, void *oldelem, void *newelem) {
  ...
  {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_setreplace): elem %p not found in set\n",  oldelem);
    qh_setprint (qhmem.ferr, "", set);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
} /* setreplace */
...
int qh_setsize(setT *set) {
  ...
  if ((size= *sizep)) {
    ...
    if (size > set->maxsize)
    {
      gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_setsize): current set size %d is greater than maximum size %d\n", size, set->maxsize);
...
setT *qh_settemp(int setsize) {
  ...
  {
    gInterface->HullPrintf (qhmem.ferr, "qh_settemp: temp set %p of %d elements, depth %d\n", newset, newset->maxsize, qh_setsize ((setT*)qhmem.tempstack));
  }
  return newset;
} /* settemp */
...
void qh_settempfree(setT **set) {
  ...
  if (stackedset != *set) {
    ...
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_settempfree): set %p (size %d) was not last temporary allocated (depth %d, set %p, size %d)\n", *set, qh_setsize(*set), qh_setsize((setT*)qhmem.tempstack)+1,	stackedset, qh_setsize(stackedset));
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
  qh_setfree (set);
} /* settempfree */
...
setT *qh_settemppop(void) {
  ...
  if (!stackedset)
  {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_settemppop): pop from empty temporary stack\n");
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
  if (qhmem.IStracing >= 5)
  {
    gInterface->HullPrintf (qhmem.ferr, "qh_settemppop: depth %d temp set %p of %d elements\n", qh_setsize((setT*)qhmem.tempstack)+1, stackedset, qh_setsize(stackedset));
  }
  return stackedset;
} /* settemppop */
...
void qh_settemppush(setT *set) {
  ...
  if (qhmem.IStracing >= 5)
  {
    gInterface->HullPrintf (qhmem.ferr, "qh_settemppush: depth %d temp set %p of %d elements\n", qh_setsize((setT*)qhmem.tempstack), set, qh_setsize(set));
  }
...
void qh_settruncate (setT *set, int size) {
  ...
  {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_settruncate): size %d out of bounds for set:\n", size);
    qh_setprint (qhmem.ferr, "", set);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
...
void qh_setzero (setT *set, int index, int size)
{
  ...
  {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_setzero): index %d or size %d out of bounds for set:\n", index, size);
    qh_setprint (qhmem.ferr, "", set);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
