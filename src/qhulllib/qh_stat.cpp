...
void qh_initstatistics (void) {
  ...
  if (!(qh_qhstat= (qhstatT *)malloc (sizeof(qhstatT)))) {
    gInterface->HullPrintf (qhmem.ferr, "qhull error (qh_initstatistics): insufficient memory\n");
  ...
  if (size_t(qhstat next) > sizeof(qhstat id)) {
	...
	if (qhstat id[i] == qhstat id[j]) {
          gInterface->HullPrintf (qhmem.ferr, "qhull error (qh_initstatistics): duplicated statistic %d at indices %d and %d\n", 
	      qhstat id[i], i, j);
	}
      }
    }
...
", 
 ...
 %6.2g max. distance for near-inside points\n", qh NEARinside);
  if (qh premerge_cos < REALmax/2) gInterface->HullPrintf (fp, "\
 %6.2g max. cosine for pre-merge angle\n", qh premerge_cos);
  if (qh PREmerge) gInterface->HullPrintf (fp, "\
 %6.2g radius of pre-merge centrum\n", qh premerge_centrum);
  if (qh postmerge_cos < REALmax/2) gInterface->HullPrintf (fp, "\
 %6.2g max. cosine for post-merge angle\n", qh postmerge_cos);
  if (qh POSTmerge) gInterface->HullPrintf (fp, "\
 ...
 %6.2g min. denominator for divisions\n\
  ...
  for (k=0; k < qh hull_dim; k++)
    gInterface->HullPrintf (fp, "%6.2e ", qh NEARzero[k]);
  gInterface->HullPrintf (fp, "\n\n");
...
#define NULLfield "       "
  ...
  if (qhstat count[id] != -1 
    ...
    gInterface->HullPrintf (fp, " *0 cnt*");
  else if (qhstat type[id] >= ZTYPEreal && qhstat count[id] == -1)
    gInterface->HullPrintf (fp, "%7.2g", qhstat stats[id].r);
  else if (qhstat type[id] >= ZTYPEreal && qhstat count[id] != -1)
    gInterface->HullPrintf (fp, "%7.2g", qhstat stats[id].r/ qhstat stats[(unsigned char)(qhstat count[id])].i);
  else if (qhstat type[id] < ZTYPEreal && qhstat count[id] == -1)
    gInterface->HullPrintf (fp, "%7d", qhstat stats[id].i);
  else if (qhstat type[id] < ZTYPEreal && qhstat count[id] != -1)
    gInterface->HullPrintf (fp, "%7.3g", (realT) qhstat stats[id].i / qhstat stats[(unsigned char)(qhstat count[id])].i);
  gInterface->HullPrintf (fp, " %s\n", qhstat doc[id]);
} /* printstatlevel */
