...
void *qh_memalloc(int insize) {
  ...
  if ((unsigned) insize <= (unsigned) qhmem.LASTsize) {
	...
	if (!(newbuffer= gInterface->HullMalloc(bufsize)))
 {
	  gInterface->HullPrintf(qhmem.ferr, "qhull error (qh_memalloc): insufficient memory\n");
	...
	*((void **)newbuffer)= qhmem.curbuffer;  /* prepend newbuffer to curbuffer 
    ...
    if (!qhmem.indextable) {
      gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_memalloc): qhmem has not been initialized.\n");
    ...
    if (!(object= gInterface->HullMalloc(outsize)))
    {
      gInterface->HullPrintf(qhmem.ferr, "qhull error (qh_memalloc): insufficient memory\n");
      qh_errexit(qhmem_ERRmem, NULL, NULL);
    }
    if (qhmem.IStracing >= 5)
      gInterface->HullPrintf (qhmem.ferr, "qh_memalloc long: %d bytes at %p\n", outsize, object);
  }
  return (object);
} /* memalloc */
...
void qh_memfree(void *object, int size) {
  ...
  if (size <= qhmem.LASTsize) {
    ...
    *freelistp= object;
    ...
    qhmem .freelong++;
    qhmem .totlong -= size;
    gInterface->HullFree(object);
    if (qhmem.IStracing >= 5)
      gInterface->HullPrintf (qhmem.ferr, "qh_memfree long: %d bytes at %p\n", size, object);
  }
} /* memfree */
...
void qh_meminit (FILE *ferr) {
  ...
  if (sizeof(void*) < sizeof(int)) {
    gInterface->HullPrintf (ferr, "qhull internal error (qh_meminit): sizeof(void*) < sizeof(int).  qset.c will not work\n");
...
void qh_meminitbuffers (int tracelevel, int alignment, int numsizes, int bufsize, int bufinit) {
  ...
  if (qhmem.ALIGNmask & ~qhmem.ALIGNmask) {
    gInterface->HullPrintf (qhmem.ferr, "qhull internal error (qh_meminit): memory alignment %d is not a power of 2\n", alignment);
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
  ...
  if (!qhmem.sizetable || !qhmem.freelists) {
    gInterface->HullPrintf(qhmem.ferr, "qhull error (qh_meminit): insufficient memory\n");
    qh_errexit (qhmem_ERRmem, NULL, NULL);
  }
  if (qhmem.IStracing >= 1)
    gInterface->HullPrintf (qhmem.ferr, "qh_meminitbuffers: memory initialized with alignment %d\n", alignment);
} /* meminitbuffers */
...
void qh_memsetup (void) {
  ...
  if (qhmem .LASTsize >= qhmem .BUFsize || qhmem.LASTsize >= qhmem .BUFinit) {
    gInterface->HullPrintf (qhmem.ferr, "qhull error (qh_memsetup): largest mem size %d is >= buffer size %d or initial buffer size %d\n",
            qhmem .LASTsize, qhmem .BUFsize, qhmem .BUFinit);
    qh_errexit(qhmem_ERRmem, NULL, NULL);
  }
  if (!(qhmem.indextable= (int *)gInterface->HullMalloc((qhmem.LASTsize+1) * sizeof(int)))) {
    gInterface->HullPrintf(qhmem.ferr, "qhull error (qh_memsetup): insufficient memory\n");
    qh_errexit(qhmem_ERRmem, NULL, NULL);
  }
...
void qh_memsize(int size) {
  ...
  if (qhmem .LASTsize) {
    gInterface->HullPrintf (qhmem .ferr, "qhull error (qh_memsize): called after qhmem_setup\n");
    qh_errexit (qhmem_ERRqhull, NULL, NULL);
  }
  ...
  if (qhmem.TABLEsize < qhmem.NUMsizes)
    ...
    gInterface->HullPrintf(qhmem.ferr, "qhull warning (memsize): free list table has room for only %d sizes\n", qhmem.NUMsizes);
} /* memsize */
...
%7d bytes per memory buffer (initially %d bytes)\n",
  ...
  if (qhmem.cntlarger) {
    gInterface->HullPrintf (fp, "%7d calls to qh_setlarger\n%7.2g     average copy size\n",
	   qhmem.cntlarger, ((float) qhmem.totlarger)/ qhmem.cntlarger);
    gInterface->HullPrintf (fp, "  freelists (bytes->count):");
  }
  for (i=0; i < qhmem.TABLEsize; i++) {
    count=0;
    for (object= qhmem .freelists[i]; object; object= *((void **)object))
      count++;
    gInterface->HullPrintf (fp, " %d->%d", qhmem.sizetable[i], count);
  }
  gInterface->HullPrintf (fp, "\n\n");
} /* memstatistics */
...
void *qh_memalloc(int insize)
{
  ...
  if (!(object= gInterface->HullMalloc(insize)))
  {
    gInterface->HullPrintf(qhmem.ferr, "qhull error (qh_memalloc): insufficient memory\n");
    qh_errexit(qhmem_ERRmem, NULL, NULL);
  }
